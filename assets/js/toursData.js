export const toursData = {
    // ==========================================================================
    // === COSTA DE PERÚ
    // ==========================================================================

    // --- Ica y Huacachina ---
    "dune-buggy-sandboard-huacachina": {
        title: "Dune Buggy & Sandboard",
        title_es: "Buggy en Dunas y Sandboard",
        price: 25,
        minPersons: 1,
        duration: "1.5 Hours",
        duration_es: "1.5 Horas",
        rating: "4.7 (8 reviews)",
        rating_es: "4.7 (8 reseñas)",
        image: '../assets/img/Costa/Ica_Huaca/Buggy_TourySandboard/14.jpg',
    },
    "vineyard-museum-oasis-tour": {
        title: "Vineyard Museum Oasis Tour",
        title_es: "Tour Viñedo Museo Oasis",
        price: 25,
        minPersons: 2,
        duration: "2.5 Hours",
        duration_es: "2.5 Horas",
        rating: "4.5 (6 reviews)",
        rating_es: "4.5 (6 reseñas)",
        image: "../assets/img/Costa/Ica_Huaca/Viñedo_MuseoOasis/16.jpg"
    },

    // --- Lima ---
    "caral-tour": {
        title: "Caral",
        title_es: "Tour a Caral",
        price: 195,
        minPersons: 2,
        duration: "Full Day",
        duration_es: "Día Completo",
        rating: "4.8 (7 reviews)",
        rating_es: "4.8 (7 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Caral/19.jpg"
    },
    "nazca-lines-from-lima-vip": {
        title: "Flights Over Nazca Lines From Lima (1-day VIP Private)",
        title_es: "Sobrevuelo Líneas de Nazca desde Lima (Privado VIP 1 día)",
        price: 430,
        minPersons: 2,
        duration: "12 Hours",
        duration_es: "12 Horas",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Sobrevuele_las_líneas_de_Nazca_desde_Pisco_1-díaServicio-Privado/35.jpg"
    },
    "lima-city-tour": {
        title: "Lima City Tour",
        title_es: "City Tour Lima",
        price: 55,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.9 (120 reviews)",
        rating_es: "4.9 (120 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Tour_de_Lima/37.jpg"
    },
    "museums-of-lima": {
        title: "Museums Of Lima",
        title_es: "Museos de Lima",
        price: 50,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.5 (7 reviews)",
        rating_es: "4.5 (7 reseñas)",
        image: "../assets/img/Costa/tours_Lima/MuseosdeLima/22.jpg"
    },
    "nightlife-in-lima": {
        title: "Nightlife In Lima",
        title_es: "Vida Nocturna en Lima",
        price: 55,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.7 (6 reviews)",
        rating_es: "4.7 (6 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Nocturna/25.jpg"
    },
    "pachacamac-tour": {
        title: "Pachacamac",
        title_es: "Tour Pachacámac",
        price: 45,
        minPersons: 2,
        duration: "12 Hours",
        duration_es: "12 Horas",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Pachacamac/31.jpg"
    },
    "peruvian-gold-museum": {
        title: "Peruvian Gold",
        title_es: "Museo Oro del Perú",
        price: 50,
        minPersons: 2,
        duration: "3 Hours",
        duration_es: "3 Horas",
        rating: "4.8 (7 reviews)",
        rating_es: "4.8 (7 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Oro_de_Perú/28.jpg"
    },

    // --- Nazca Lines ---
    "cantalloc-aqueducts": {
        title: "Cantalloc Aqueducts",
        title_es: "Acueductos de Cantalloc",
        price: 20,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.5 (8 reviews)",
        rating_es: "4.5 (8 reseñas)",
        image: "../assets/img/Costa/LineasNazca/Acueductos_de_Cantalloc/40.jpg"
    },
    "chauchilla-cemetery": {
        title: "Chauchilla Cemetery",
        title_es: "Cementerio Chauchilla",
        price: 25,
        minPersons: 2,
        duration: "2.5 Hours",
        duration_es: "2.5 Horas",
        rating: "4.7 (6 reviews)",
        rating_es: "4.7 (6 reseñas)",
        image: "../assets/img/Costa/LineasNazca/Cemetario_Chaucilla/43.jpg"
    },
    "nazca-lines-from-lima-vip": {
        title: "Flights Over Nazca Lines From Paracas (1-day VIP Private)",
        title_es: "Sobrevuelo Líneas de Nazca desde Paracas (Privado VIP 1 día)",
        price: 430,
        minPersons: 2,
        duration: "12 Hours",
        duration_es: "12 Horas",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/Costa/tours_Lima/Sobrevuele_las_líneas_de_Nazca_desde_Pisco_1-díaServicio-Privado/34.jpg"
    },
    "nazca-lines-flight": {
        title: "Nazca Lines",
        title_es: "Líneas de Nazca",
        price: 120,
        minPersons: 2,
        duration: "35 Mins",
        duration_es: "35 Minutos",
        rating: "4.8 (8 reviews)",
        rating_es: "4.8 (8 reseñas)",
        image: "../assets/img/Costa/LineasNazca/Lineas_de_Nazca/46.jpg"
    },
    "nazca-lines-from-paracas": {
        title: "Nazca Lines from Paracas",
        title_es: "Líneas de Nazca desde Paracas",
        price: 280,
        minPersons: 2,
        duration: "1 Hour 45 Mins",
        duration_es: "1 Hora 45 Min",
        rating: "4.6 (7 reviews)",
        rating_es: "4.6 (7 reseñas)",
        image: "../assets/img/Costa/LineasNazca/Líneas_de_Nazca_desde_Paracas/48.jpg"
    },
    "pampas-galeras-reserve": {
        title: "Pampas Galeras Reserve",
        title_es: "Reserva Pampas Galeras",
        price: 60,
        minPersons: 2,
        duration: "6 Hours",
        duration_es: "6 Horas",
        rating: "4.5 (6 reviews)",
        rating_es: "4.5 (6 reseñas)",
        image: "../assets/img/Costa/LineasNazca/ReservaPampasGaleras/49.jpg"
    },

    // --- North Coast ---
    "chan-chan-tour": {
        title: "Chan Chan",
        title_es: "Chan Chan",
        price: 0, 
        minPersons: 2,
        duration: "3 Hours",
        duration_es: "3 Horas",
        rating: "4.7 (8 reviews)",
        rating_es: "4.7 (8 reseñas)",
        image: "../assets/img/Costa/Costa_Norte/ChanChan/1.jpg"
    },
    "mangroves-of-tumbes": {
        title: "Mangroves Of Tumbes",
        title_es: "Manglares de Tumbes",
        price: 100,
        minPersons: 2,
        duration: "12 Hours",
        duration_es: "12 Horas",
        rating: "4.8 (7 reviews)",
        rating_es: "4.8 (7 reseñas)",
        image: "../assets/img/Costa/Costa_Norte/Manglares_de_Tumbes/7.jpg"
    },
    "snorkel-with-turtles": {
        title: "Snorkel With Turtles",
        title_es: "Snorkel con Tortugas",
        price: 35,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.6 (6 reviews)",
        rating_es: "4.6 (6 reseñas)",
        image: "../assets/img/Costa/Costa_Norte/Snorkel_con_las_tortugas/10.jpg"
    },
    "temple-of-the-moon": {
        title: "Temple Of The Moon",
        title_es: "Huaca de la Luna",
        price: 25,
        minPersons: 2,
        duration: "3 Hours",
        duration_es: "3 Horas",
        rating: "4.5 (8 reviews)",
        rating_es: "4.5 (8 reseñas)",
        image: "../assets/img/Costa/Costa_Norte/Huaca_de_la_Luna/4.jpg"
    },

    // --- Paracas y Islas Ballestas ---
    "biking-paracas": {
        title: "Biking Paracas",
        title_es: "Ciclismo en Paracas",
        price: 18,
        minPersons: 1,
        duration: "5 Hours",
        duration_es: "5 Horas",
        rating: "4.7 (7 reviews)",
        rating_es: "4.7 (7 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/LaReservaParacasBici/67.jpg"
    },
    "cruise-member-islas-ballestas-first-class": {
        title: "Cruise Member Islas Ballestas First Class",
        title_es: "Islas Ballestas Primera Clase (Cruceristas)",
        price: 110,
        minPersons: 1,
        duration: "5-6 Hours",
        duration_es: "5-6 Horas",
        rating: "4.8 (6 reviews)",
        rating_es: "4.8 (6 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/Tour_primera_Islas_ballestas_miembros_cruceros/76.jpg"
    },
    "cruise-member-islas-ballestas-standard-tour": {
        title: "Cruise Member Islas Ballestas Standard Tour",
        title_es: "Islas Ballestas Estándar (Cruceristas)",
        price: 85,
        minPersons: 1,
        duration: "5-6 Hours",
        duration_es: "5-6 Horas",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/Tour_Islas_ballestas_miembros_ruceros_standard_tours/73.jpg"
    },
    "cruise-shore-excursions": {
        title: "Cruise Shore Excursions",
        title_es: "Excursiones en Costa para Cruceros",
        price: 0, 
        minPersons: 1,
        duration: "Varies",
        duration_es: "Varía",
        rating: "4.5 (7 reviews)",
        rating_es: "4.5 (7 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/ExcursionesCrucero/58.jpg"
    },
    "dune-buggy-sandboard-paracas": {
        title: "Dune Buggy & Sandboard",
        title_es: "Buggy y Sandboard",
        price: 50,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.7 (6 reviews)",
        rating_es: "4.7 (6 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/Tubular_Arenero_Sandboard/82.jpg"
    },
    "islas-ballestas": {
        title: "Islas Ballestas",
        title_es: "Islas Ballestas",
        price: 17,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.8 (8 reviews)",
        rating_es: "4.8 (8 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/IslasBallestas/60.jpg"
    },
    "islas-ballestas-first-class": {
        title: "Islas Ballestas First Class",
        title_es: "Islas Ballestas Primera Clase",
        price: 50,
        minPersons: 1,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.6 (7 reviews)",
        rating_es: "4.6 (7 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/IslasBallestasFirstClass/63.jpg"
    },
    "kayak-paracas-bay": {
        title: "Kayak Paracas Bay",
        title_es: "Kayak Bahía de Paracas",
        price: 20,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.5 (6 reviews)",
        rating_es: "4.5 (6 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/Kayak en Bahia de Paracas/Kayak-1.jpg"
    },
    "nazca-lines-from-paracas": {
        title: "Nazca Lines from Paracas",
        title_es: "Líneas de Nazca desde Paracas",
        price: 280,
        minPersons: 2,
        duration: "1 Hour 45 Mins",
        duration_es: "1 Hora 45 Min",
        rating: "4.6 (7 reviews)",
        rating_es: "4.6 (7 reseñas)",
        image: "../assets/img/Costa/LineasNazca/Líneas_de_Nazca_desde_Paracas/48.jpg"
    },
    "paracas-national-reserve": {
        title: "Paracas National Reserve",
        title_es: "Reserva Nacional de Paracas",
        price: 29,
        minPersons: 2,
        duration: "3 Hours",
        duration_es: "3 Horas",
        rating: "4.8 (210 reviews)",
        rating_es: "4.8 (210 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/La_Reserva_Nacional_Paracas/70.jpg"
    },
    "private-tour-paracas": {
        title: "Private Tour",
        title_es: "Tour Privado",
        price: 300,
        minPersons: 2,
        duration: "2 Hours",
        duration_es: "2 Horas",
        rating: "4.7 (8 reviews)",
        rating_es: "4.7 (8 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/Tour_Privado/79.jpg"
    },
    "yacht-charter-paracas": {
        title: "Yacht Charter",
        title_es: "Alquiler de Yate",
        price: 360,
        minPersons: 2,
        duration: "2 Hours (min)",
        duration_es: "2 Horas (mín)",
        rating: "4.8 (7 reviews)",
        rating_es: "4.8 (7 reseñas)",
        image: "../assets/img/Costa/tours_Paracas/CharterYate/55.jpg"
    },

    // ==========================================================================
    // === SIERRA (THE HIGHLANDS)
    // ==========================================================================

    // --- Arequipa ---
    "arequipa-city-tour": {
        title: "Arequipa City Tour",
        title_es: "Tour Ciudad de Arequipa",
        price: 60,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.6 (6 reviews)",
        rating_es: "4.6 (6 reseñas)",
        image: "../assets/img/SIerra/Arequipa/tour_Arequipa/85.jpg"
    },
    "colca-canyon-tour": {
        title: "Colca Canyon Tour",
        title_es: "Tour Cañón del Colca",
        price: 36,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.5 (8 reviews)",
        rating_es: "4.5 (8 reseñas)",
        image: "../assets/img/SIerra/Arequipa/Tour_CañonColca/88.jpg"
    },
    
    // --- Cuzco ---
    "cuzco-city-tour": {
        title: "Cuzco City Tour",
        title_es: "Tour Ciudad de Cusco",
        price: 60,
        minPersons: 2,
        duration: "4 Hours",
        duration_es: "4 Horas",
        rating: "4.7 (7 reviews)",
        rating_es: "4.7 (7 reseñas)",
        image: "../assets/img/SIerra/Cuzco/Tour-de-Cuzco/109.jpg"
    },
    "inca-bike-jungle-trek": {
        title: "Inca Bike & Jungle Trek",
        title_es: "Inca Jungle Trek y Bicicleta",
        price: 260,
        minPersons: 2,
        duration: "4 Days (3 Nights)",
        duration_es: "4 Días (3 Noches)",
        rating: "4.8 (6 reviews)",
        rating_es: "4.8 (6 reseñas)",
        image: "../assets/img/SIerra/Cuzco/IncaBikeJungleTrek/100.jpg"
    },
    "inca-trail-classic-trek": {
        title: "Inca Trail Classic Trek",
        title_es: "Camino Inca Clásico",
        price: 260,
        minPersons: 2,
        duration: "4 Days (3 Nights)",
        duration_es: "4 Días (3 Noches)",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/SIerra/Cuzco/Clásico_trekenCamino_Inca/97.jpg"
    },
    "inca-trail-salkantay-trek": {
        title: "Inca Trail Salkantay Trek",
        title_es: "Camino Inca Salkantay Trek",
        price: 260,
        minPersons: 2,
        duration: "4 Days (3 Nights)",
        duration_es: "4 Días (3 Noches)",
        rating: "4.5 (7 reviews)",
        rating_es: "4.5 (7 reseñas)",
        image: "../assets/img/SIerra/Cuzco/IncaTrailSalkantayTrek/103.jpg"
    },
    "machu-picchu-day-trip": {
        title: "Machu Picchu Day Trip",
        title_es: "Viaje de un día a Machu Picchu",
        price: 330,
        minPersons: 2,
        duration: "14 Hours",
        duration_es: "14 Horas",
        rating: "4.7 (6 reviews)",
        rating_es: "4.7 (6 reseñas)",
        image: "../assets/img/SIerra/Cuzco/Machu-Picchu-en-Un-Día/106.jpg"
    },
    "white-water-rafting-cuzco": {
        title: "White Water Rafting",
        title_es: "Canotaje en Aguas Bravas",
        price: 260,
        minPersons: 2,
        duration: "1 Day",
        duration_es: "1 Día",
        rating: "4.8 (8 reviews)",
        rating_es: "4.8 (8 reseñas)",
        image: "../assets/img/SIerra/Cuzco/Canotaje/94.jpg"
    },

    // --- Lake Titicaca ---
    "lake-titicaca-boat-tour": {
        title: "Lake Titicaca Boat Tour",
        title_es: "Tour Lago Titicaca en Bote",
        price: 60,
        minPersons: 2,
        duration: "Over 10 Hours",
        duration_es: "Más de 10 Horas",
        rating: "4.6 (7 reviews)",
        rating_es: "4.6 (7 reseñas)",
        image: "../assets/img/SIerra/Lago_Titi/Lago-Titicaca-en-Lancha/91.jpg"
    },

    // ==========================================================================
    // === SELVA (RAINFOREST)
    // ==========================================================================

    // --- Manu Reserve & Tambopata ---
    "manu-reserve-tour": {
        title: "Manu Reserve",
        title_es: "Reserva del Manu",
        price: 100,
        minPersons: 2,
        duration: "12 Hours",
        duration_es: "12 Horas",
        rating: "4.5 (6 reviews)",
        rating_es: "4.5 (6 reseñas)",
        image: "../assets/img/Selva_Tropical/Parque_Manu/112.jpg"
    },
    "tambopata-reserve-tour": {
        title: "Tambopata Reserve",
        title_es: "Reserva Tambopata",
        price: 260,
        minPersons: 2,
        duration: "5 Days (4 Nights)",
        duration_es: "5 Días (4 Noches)",
        rating: "4.7 (8 reviews)",
        rating_es: "4.7 (8 reseñas)",
        image: "../assets/img/Selva_Tropical/Tambopata/Tambopata/115.jpg"
    },
    
    // ==========================================================================
    // === EXCURSIONES PARA CRUCEROS (SHORE EXCURSIONS)
    // ==========================================================================
    
    // --- Desde Callao ---
    "shore-excursions-larco-museum": {
        title: "Shore Excursions to Larco Museum",
        title_es: "Excursión en Costa al Museo Larco",
        price: 75,
        minPersons: 2,
        duration: "6 Hours",
        duration_es: "6 Horas",
        rating: "4.8 (7 reviews)",
        rating_es: "4.8 (7 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Callao/Excursiones_tierra_MuseoLarco/127.jpg"
    },
    "shore-excursions-pachacamac": {
        title: "Shore Excursions to Pachacamac Archaeological Sites",
        title_es: "Excursión en Costa a los Sitios Arqueológicos de Pachacamac",
        price: 85,
        minPersons: 2,
        duration: "8 Hours",
        duration_es: "8 Horas",
        rating: "4.6 (6 reviews)",
        rating_es: "4.6 (6 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Callao/Excursiones_tierra_Sitios_arqueológicos_Pachacamac/130.jpg"
    },
    
    // --- Desde Salaverry ---
    "shore-excursions-chan-chan": {
        title: "Shore Excursions to Chan Chan",
        title_es: "Excursión en Costa a Chan Chan",
        price: 85,
        minPersons: 2,
        duration: "6 Hours",
        duration_es: "6 Horas",
        rating: "4.5 (8 reviews)",
        rating_es: "4.5 (8 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Salaverry/Excursiones_tierra_ChanChan/142.jpg"
    },
    
    // --- Desde Pisco / Paracas ---
    "nazca-lines-flight-from-pisco-cruise": {
        title: "Nazca Lines Flight from Pisco Cruise Terminal",
        title_es: "Sobrevuelo Nazca desde Terminal Pisco",
        price: 380,
        minPersons: 2,
        duration: "6 Hours",
        duration_es: "6 Horas",
        rating: "4.7 (7 reviews)",
        rating_es: "4.7 (7 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Paracas/Vuelo_las_Líneas_Nazca/139.jpg"
    },
    "shore-excursions-islas-ballestas-from-pisco": {
        title: "Shore Excursions to Islas Ballestas",
        title_es: "Excursión en Costa a las Islas Ballestas",
        price: 100,
        minPersons: 2,
        duration: "6 Hours",
        duration_es: "6 Horas",
        rating: "4.8 (6 reviews)",
        rating_es: "4.8 (6 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Paracas/Excursiones_tierra_IslasBallestas/133.jpg"
    },
    "shore-excursions-tambo-colorado": {
        title: "Shore Excursions to Tambo Colorado",
        title_es: "Excursión en Costa a Tambo Colorado",
        price: 95,
        minPersons: 2,
        duration: "8 Hours",
        duration_es: "8 Horas",
        rating: "4.6 (8 reviews)",
        rating_es: "4.6 (8 reseñas)",
        image: "../assets/img/Excursiones/Terminal_Paracas/Excursiones_tierra_TamboColorado/136.jpg"
    },

    // ==========================================================================
    // === PAQUETES DE PERÚ (PERU PACKAGES)
    // ==========================================================================
    "machu-picchu-sacred-valley-7-days": {
        title: "Machu Picchu & The Secret Valley of the Incas",
        title_es: "Disfruta la aventura de conocer Machu Picchu y el Valle Sagrado de los Incas",
        price: 1600,
        minPersons: 1,
        duration: "6 Days",
        duration_es: "6 Días",
        rating: "4.5 (7 reviews)",
        rating_es: "4.5 (7 reseñas)",
        image: "../assets/img/Paquetes_Peru/Machu_Picchu_Valle/MACHUPICCHU_VALLE_SECRETO_LOS_INCAS_7_días/118.jpg"
    },
    "mysteries-of-south-coast-2-days": {
        title: "Nazca Lines Flight & South Coast",
        title_es: "Vuelo Líneas de Nazca y Costa Sur",
        price: 565,
        minPersons: 2,
        duration: "2 Days / 1 Night",
        duration_es: "2 Días / 1 Noche",
        rating: "4.7 (6 reviews)",
        rating_es: "4.7 (6 reseñas)",
        image: "../assets/img/Paquetes_Peru/Misterios_Costa/Misterios_Costa_Sur/121.jpg"
    },
    "mysteries-of-south-coast-vip-upgrade": {
        title: "Nazca Lines Flight & South Coast - Upgrade to VIP",
        title_es: "Vuelo Líneas de Nazca y Costa Sur - Upgrade a VIP",
        price: 886,
        minPersons: 2,
        duration: "2 Days / 1 Night",
        duration_es: "2 Días / 1 Noche",
        rating: "4.8 (8 reviews)",
        rating_es: "4.8 (8 reseñas)",
        image: "../assets/img/Paquetes_Peru/Misterios_Costa/Servicio_Mejorado_VIP/124.jpg"
    },
    //prueba
    "donaciondedonaciones": {
        title: "Donacion de donaciones",
        title_es: "Donacion de donaciones",
        price: 1,
        minPersons: 1,
        duration: "Donacion",
        duration_es: "Donacion ",
        rating: "4.8 (8 reviews)",
        rating_es: "4.8 (8 reseñas)",
        image: ""
    }
};